...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/08 13:21:55 by mferracc          #+#    #+#             */
/*   Updated: 2023/03/12 11:14:05 by mferracc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	start_play(t_game *game)
{
	...
	check_screen(game);
	game->mlx_win = mlx_new_window(game->mlx_ptr, game->map_info->width * SIZE, \
			game->map_info->height * SIZE, "Blobby's World");
	if (!game->mlx_win)
		return (0);
	if (!load_img(game))
